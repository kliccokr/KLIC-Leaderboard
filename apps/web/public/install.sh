...
# 5. OTel env setup + Start daemon
OTEL_ENV_FILE="$HOME/.claude/klic-otel.env"
OTEL_MARK="# klic-leaderboard OTel"
CONFIG_FILE="$HOME/.klic/leaderboard/config.json"

if [ "$LOGIN_OK" = true ] && [ -f "$CONFIG_FILE" ]; then
  API_KEY=$(grep -o '"apiKey"[[:space:]]*:[[:space:]]*"[^"]*"' "$CONFIG_FILE" | sed -E 's/.*"apiKey"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/')
  SERVER_URL=$(grep -o '"serverUrl"[[:space:]]*:[[:space:]]*"[^"]*"' "$CONFIG_FILE" | sed -E 's/.*"serverUrl"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/')
  SERVER_URL="${SERVER_URL:-https://use.klic.co.kr}"

  if [ -n "$API_KEY" ]; then
    mkdir -p "$HOME/.claude"
    HOSTNAME_ESC=$(hostname 2>/dev/null | tr -c 'A-Za-z0-9._-' '_' | cut -c1-80)
    cat > "$OTEL_ENV_FILE" <<OTEL_EOF
# $OTEL_MARK — generated by install.sh, safe to regenerate
...
export OTEL_RESOURCE_ATTRIBUTES="service.instance.id=$HOSTNAME_ESC"
OTEL_EOF
    chmod 600 "$OTEL_ENV_FILE"

    # Idempotent injection into shell rc files
    for rc in "$HOME/.zshrc" "$HOME/.bashrc"; do
      [ -f "$rc" ] || continue
      if ! grep -q "$OTEL_MARK" "$rc" 2>/dev/null; then
        printf '\n%s\n[ -f "%s" ] && source "%s"\n' "$OTEL_MARK" "$OTEL_ENV_FILE" "$OTEL_ENV_FILE" >> "$rc"
      fi
    done
    log "   -> OTel 환경변수 설정 완료 (${CYAN}$OTEL_ENV_FILE${NC})"
    log "   -> 새 터미널을 열거나 ${CYAN}source $OTEL_ENV_FILE${NC} 실행 후 Claude Code 재시작"
  fi
fi
